...
tot_area <- off_area %>%
  ...
  group_by(Season) %>%
  mutate(delta_mean = 100 * (Area - mean(Area, na.rm = T)) / mean(Area, na.rm = T))
...
data_mod <- rbind(data_srv_dry, data_srv_wet) %>%
  ...
  group_by(Season) %>%
  mutate(delta_mean_mod = 100 * (area_mod - mean(area_mod, na.rm = T)) / mean(area_mod, na.rm = T))
