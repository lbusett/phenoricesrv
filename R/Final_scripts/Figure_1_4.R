...
Figure_1 <- ggplot(tot_area, aes(x = factor(Year), y = Area, fill = Season)) +
...
data_all$year_short <- stringr::str_sub(data_all$Year ,3, 4)





Figure_4 <- ggplot(data_all,
                   aes(x = Off_Area, y = PhenoRice_Area, color = Season, label = year_short)) +
  geom_point(pch = 24, size = 1.8, aes(fill = Season)) + theme_light() +
  geom_text(nudge_y = 1000, show.legend = FALSE) +
  geom_smooth(method = "lm", show.legend = FALSE, linetype = 3, lwd = 0.5) +
  geom_hline(yintercept = 0, colour = "grey30", size = 0.5) +
  geom_vline(xintercept = 0, colour = "grey30", size = 0.5) +
  xlim(-1000, 40000) + ylim(-1000,40000) +
  scale_colour_manual(values = c("darkred", "darkgreen")) +
  scale_fill_manual(values = c("darkred", "darkgreen"))+
  ylab("PhenoRice Estimated Rice Area [ha]") + xlab("Official Reported Rice Area [ha]") +
  geom_abline(slope = 1, colour = "grey70", linetype = 2) +
  xlim(0,45000) + ylim(0,45000) +
  theme(legend.box.background = element_rect(),
        legend.box.margin = margin(1,1,1,1))
Figure_4

ggsave(filename = "/home/lb/Google_Drive/IREA/MI_Conference&paper/Busetto_PhenoRice_SRV/Manuscript/Figures/Figure_4.png",
       dpi = 400, width = 6.57, height = 4.23, plot = Figure_4)

data_all <- ungroup(data_all)
Figure_4b <- ggplot(data_all,  aes(x = Off_Area, y = PhenoRice_Area, label = year_short)) +
  geom_point(pch = 24, size = 1.8, aes(fill = Season, color = Season)) + theme_light() +
  geom_text(nudge_y = 1000, show.legend = FALSE) +
  geom_smooth(method = "lm", show.legend = FALSE, linetype = 3, lwd = 0.5) +
  geom_hline(yintercept = 0, colour = "grey30", size = 0.5) +
  geom_vline(xintercept = 0, colour = "grey30", size = 0.5) +
  ...
  scale_colour_manual(values = c("darkred", "darkgreen")) +
  scale_fill_manual(values = c("darkred", "darkgreen"))+
  ylab("PhenoRice Detected Rice Area [ha]") + xlab("Official Reported Rice Area [ha]") +
  geom_abline(slope = 1, colour = "grey70", linetype = 2) +
  xlim(-5000, 40000) + ylim(-5000,40000) +
  theme(legend.box.background = element_rect(),
        legend.box.margin = margin(1,1,1,1))
Figure_4b

ggsave(filename = "/home/lb/Google_Drive/IREA/MI_Conference&paper/Busetto_PhenoRice_SRV/Manuscript/Figures/Figure_4b.png",
       dpi = 400, width = 6.57, height = 4.23, plot = Figure_4b)


plot <- lb_plotregr(data_all, "Off_Area", "PhenoRice_Area",
                    common_range = T)  +
  geom_point(pch = 24, size = 1.8, aes(fill = Season, color = Season)) +
  theme_light() +
  xlim(-1000, 40000) + ylim(-1000,40000) +
  scale_colour_manual(values = c("darkred", "darkgreen")) +
  scale_fill_manual(values = c("darkred", "darkgreen"))+
